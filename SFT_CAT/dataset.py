...
@dataclass
class DataArguments:
    data_path: str = field(default='/home/qilang/PythonProjects/AVLLM/MLLM_Cat/train-music-and-avqa-generic-llama.json',
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        image = torch.zeros((60 if data_type == 'video' else 1, 1024), dtype=torch.float16)
        audio = torch.zeros((60 if data_type == 'video' else 1, 1024), dtype=torch.float16)
        ...
        try:
            ...
            image = np.load(feature_path_v) # <N, 768> float16 取第24帧
            image = torch.from_numpy(image)
            if data_type == 'image' and len(image.shape) == 1: # <768>
                image = image.unsqueeze(0)
            audio = np.load(feature_path_a) # <N, 768> float16 取第24帧
        ...
        data_dict = preprocess(
            ...
            has_image=True)
        if isinstance(i, int):
            data_dict = dict(input_ids=data_dict["input_ids"][0], #input_ids只包含文本token
...
@dataclass
class DataCollatorForSupervisedDataset(object):  #修改输入数据
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0]:
            ...
            if all(x is not None and x.shape == images[0].shape for x in images):
                batch['images'] = torch.stack(images) #列表转batch
            else:
                batch['images'] = images
