...
from mllmcatllm_trainer import MLLMCatTrainer
...
from mm_utils import print_trainable_parameters
...
class ModelArguments:
    ...
    pretrain_CA: Optional[str] = field(default=None)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    tf32: bool = True
    bf16: bool = True
    output_dir: str = field(default='./ckpt')
...
def save_model_trainer_projection(trainer: transformers.Trainer,output_dir: str):
    ...
    # Only save Adapter
    keys_to_match1 = ['mm_projector_v']
    keys_to_match2 = ['mm_projector_a']

    weight_to_save1 = get_mm_adapter_state_maybe_zero_3(trainer.model.named_parameters(), keys_to_match1)
    weight_to_save2 = get_mm_adapter_state_maybe_zero_3(trainer.model.named_parameters(), keys_to_match2)
    trainer.model.config.save_pretrained(output_dir)

    mm_projector_folder1 = os.path.join(output_dir, "mm_projector_v")
    os.makedirs(mm_projector_folder1, exist_ok=True)

    mm_projector_folder2 = os.path.join(output_dir, "mm_projector_a")
    os.makedirs(mm_projector_folder2, exist_ok=True)
    torch.save(weight_to_save1, os.path.join(mm_projector_folder1, 'mm_projector_v.bin'))
    torch.save(weight_to_save2, os.path.join(mm_projector_folder2, 'mm_projector_a.bin'))
...
def train():
    ...
    model.get_model().initialize_av_modules(model_args=model_args)
    ...
    model.get_model().mm_projector_v.to(dtype=compute_dtype, device=training_args.device)
    ...
    trainer = MLLMCatTrainer(model=model,
                    tokenizer=tokenizer,
                    args=training_args,
                    **data_module)
    ...
    save_model_trainer_projection(trainer=trainer,output_dir='./ckpt-adapter')
